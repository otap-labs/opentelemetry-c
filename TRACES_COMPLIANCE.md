...
| Environment configuration | Implemented | `OTEL_SDK_DISABLED`, resource/service variables, built-in sampler variables, supported BSP variables, and generic/signal-specific OTLP endpoint/protocol/header/timeout/compression settings. Programmatic configuration wins; see `docs/ENVIRONMENT.md` for pinned synchronous timeout limitations. |
| Span limits | Implemented | Max attributes/events/links per span and per-event/per-link attributes via `otel_sdk_builder_set_span_limits`; environment settings and then spec defaults apply without a programmatic override; overflow items are dropped by the SDK. |
